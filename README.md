...
`tar-to-zip` can transform the files as they are being processed using `options`: `{map: (fileHeader) => fileHeader}`
...
- `options` - **object** with properties:
  ...
  - `map` - map function to transform or filter files `(fileHeader) => fileHeader`. E.g. Change fileHeader.name to rename a file. Return `false` to filter out a file.
